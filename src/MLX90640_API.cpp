...
static void ExtractVDDParameters(uint16 *eeData, paramsMLX90640 *mlx90640);
static void ExtractPTATParameters(uint16 *eeData, paramsMLX90640 *mlx90640);
static void ExtractGainParameters(uint16 *eeData, paramsMLX90640 *mlx90640);
static void ExtractTgcParameters(uint16 *eeData, paramsMLX90640 *mlx90640);
static void ExtractResolutionParameters(uint16 *eeData, paramsMLX90640 *mlx90640);
static void ExtractKsTaParameters(uint16 *eeData, paramsMLX90640 *mlx90640);
static void ExtractKsToParameters(uint16 *eeData, paramsMLX90640 *mlx90640);
static void ExtractAlphaParameters(uint16 *eeData, paramsMLX90640 *mlx90640);
static void ExtractOffsetParameters(uint16 *eeData, paramsMLX90640 *mlx90640);
static void ExtractKtaPixelParameters(uint16 *eeData, paramsMLX90640 *mlx90640);
static void ExtractKvPixelParameters(uint16 *eeData, paramsMLX90640 *mlx90640);
static void ExtractCPParameters(uint16 *eeData, paramsMLX90640 *mlx90640);
static void ExtractCILCParameters(uint16 *eeData, paramsMLX90640 *mlx90640);
static int ExtractDeviatingPixels(uint16 *eeData, paramsMLX90640 *mlx90640);
static int CheckAdjacentPixels(uint16 pix1, uint16 pix2);  
static float GetMedian(float *values, int n);
static int IsPixelBad(uint16 pixel,paramsMLX90640 *params);
static int ValidateFrameData(uint16 *frameData);
static int ValidateAuxData(uint16 *auxData);
  
int MLX90640_DumpEE(uint8_t slaveAddr, uint16 *eeData)
{
...
int MLX90640_TriggerMeasurement(uint8_t slaveAddr)
{
    ...
    error = MLX90640_I2CGeneralReset(); // MLX90640_I2CGeneralReset() 官方暂时没写此函数
...
int MLX90640_GetFrameData(uint8_t slaveAddr, uint16 *frameData)
{
    uint16 dataReady = 0;
...
int MLX90640_ExtractParameters(uint16 *eeData, paramsMLX90640 *mlx90640)
{
    int error = 0;
    ...
    error = ExtractDeviatingPixels(eeData, mlx90640); // 提取检测的坏点
...
int MLX90640_SetResolution(uint8_t slaveAddr, uint8_t resolution)
{
    ...
    value = ((uint16)resolution << MLX90640_CTRL_RESOLUTION_SHIFT);
    value &= ~MLX90640_CTRL_RESOLUTION_MASK;
    
    error = MLX90640_I2CRead(slaveAddr, MLX90640_CTRL_REG, 1, &controlRegister1);
    printf("SetResolution error:%d\n", error);
    if(error == MLX90640_NO_ERROR)
    {
...
int MLX90640_GetCurResolution(uint8_t slaveAddr)
{
    ...
    error = MLX90640_I2CRead(slaveAddr, MLX90640_CTRL_REG, 1, &controlRegister1);
    printf("GetCurResolution error:%d\n", error);
    if(error != MLX90640_NO_ERROR)
    {
        return error;
    }    
...
int MLX90640_SetRefreshRate(uint8_t slaveAddr, uint8_t refreshRate)
{
    ...
    value = ((uint16)refreshRate << MLX90640_CTRL_REFRESH_SHIFT);
...
int MLX90640_SetInterleavedMode(uint8_t slaveAddr)
{
    ...
    error = MLX90640_I2CRead(slaveAddr, MLX90640_CTRL_REG, 1, &controlRegister1);
    printf("SetInterleavedMode error:%d\n", error);
    if(error == 0)
    {
...
int MLX90640_GetSubPageNumber(uint16 *frameData)
{
    return frameData[833];    
}    
...
static void ExtractTgcParameters(uint16 *eeData, paramsMLX90640 *mlx90640)
{
    mlx90640->tgc = (int8)MLX90640_LS_BYTE(eeData[60]) / 32.0f;
}
...
static void ExtractKsTaParameters(uint16 *eeData, paramsMLX90640 *mlx90640)
{   
    mlx90640->KsTa = (int8)MLX90640_MS_BYTE(eeData[60]) / 8192.0f;
}
...
static void ExtractKsToParameters(uint16 *eeData, paramsMLX90640 *mlx90640)
{
    ...
    mlx90640->ksTo[0] = (int8)MLX90640_LS_BYTE(eeData[61]) / (float)KsToScale;
    mlx90640->ksTo[1] = (int8)MLX90640_MS_BYTE(eeData[61]) / (float)KsToScale;
    mlx90640->ksTo[2] = (int8)MLX90640_LS_BYTE(eeData[62]) / (float)KsToScale;
    mlx90640->ksTo[3] = (int8)MLX90640_MS_BYTE(eeData[62]) / (float)KsToScale;
...
static void ExtractKtaPixelParameters(uint16 *eeData, paramsMLX90640 *mlx90640)
{
    ...
    KtaRC[0] = (int8)MLX90640_MS_BYTE(eeData[54]);;
    KtaRC[2] = (int8)MLX90640_LS_BYTE(eeData[54]);;
    KtaRC[1] = (int8)MLX90640_MS_BYTE(eeData[55]);;
    KtaRC[3] = (int8)MLX90640_LS_BYTE(eeData[55]);;
...
static void ExtractCPParameters(uint16 *eeData, paramsMLX90640 *mlx90640)
{
    ...
    cpKta = (int8)MLX90640_LS_BYTE(eeData[59]);
    
    ktaScale1 = MLX90640_NIBBLE2(eeData[56]) + 8;    
    mlx90640->cpKta = cpKta / POW2(ktaScale1);
    
    cpKv = (int8)MLX90640_MS_BYTE(eeData[59]);
...
//------------------------------------------------------------------------------
 ...
 static int CheckAdjacentPixels(uint16 pix1, uint16 pix2)
 {
     ...
     uint16 lp1 = pix1 >> 5;
     uint16 lp2 = pix2 >> 5;
     uint16 cp1 = pix1 - (lp1 << 5);
     uint16 cp2 = pix2 - (lp2 << 5);
     pixPosDif = lp1 - lp2;
