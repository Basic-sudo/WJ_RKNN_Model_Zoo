...
bool SerialPort::open(const std::string &path, const OpenOptions &options)
{
    ...
    tcflush(_tty_fd, TCIOFLUSH);        // 清除所有正在发生的I/O数据(清空缓冲区)
    _is_open = true;
    return _is_open;
}
...
extern uint8 G_receive;
E_SERIAL_PORT_STATE G_serialPortState = e_port_init;
void SerialPort::openThread()
{
    thread serverThread(
        [&]()->void {
            ...
            while (_is_open) {
                memset(buffer, 0, 1);
                ret = this->read(buffer, 1);
                cout << "in serverThread" << endl;
                if (ret > 0) {
                    cout << "success: ret=" << ret << endl;
                	cout << "success: buffer=" << static_cast<int>(buffer[0]) << endl;
                    G_receive = static_cast<int>(buffer[0]);
                }
            }
            return;
        }
...
int SerialPortInit(IN SerialPort& port, IN string path)
{
	...
	port.open(path, SerialPort::defaultOptions);
	if (port.isOpen()) {
